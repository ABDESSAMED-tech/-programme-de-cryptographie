...
def Compteur(c):
    ...
    return a,b
############################################ 
def que_des_majuscules(texte):
    texte=sup_espace(texte)
    # print(texte)
    chaine = ""
...
def trouver_intervalle_anormal(texte, seuil_ic=0.078, max_intervalle=100):
    intervall=""
    for intervalle in range(1, max_intervalle + 1):
        ic_actuel = ic_par_intervalle(texte, intervalle)
        intervall+=f"Intervalle: {intervalle}, IC: {ic_actuel:.5f}+<br>"
        print(f"Intervalle: {intervalle}, IC: {ic_actuel:.5f}")  # Affiche l'intervalle et l'IC actuel
        if ic_actuel > seuil_ic:
            return intervalle,intervall
    return None,None
...
def trouver_cles_potentielles(texte_chiffre, longueur_cle, dic):
    groupes_possibles = []
    extrait=""
    for i in range(longueur_cle):
        sous_groupe = texte_chiffre[i::longueur_cle]
        frequences = Counter(sous_groupe)
        lettre_choisie = None

        for lettre, freq in frequences.items():
            if lettre_choisie is None or freq > frequences[lettre_choisie] or (freq == frequences[lettre_choisie] and lettre >= lettre_choisie):
                lettre_choisie = lettre

        print(f"lettre={lettre_choisie}")

        decalage = (dic[lettre_choisie] - dic['E']) % 26
        lettre_cle = [k for k, v in dic.items() if v == (dic['A'] + decalage)][0]
        extrait+=f"L'extrait {i+1} (commençant par '{sous_groupe[0]}'), la lettre la plus fréquente est '{lettre_choisie}'. La lettre {i+1} de la clé est : '{lettre_cle}' \n"
        print(f"L'extrait {i+1} (commençant par '{sous_groupe[0]}'), la lettre la plus fréquente est '{lettre_choisie}'. La lettre {i+1} de la clé est : '{lettre_cle}'")

        groupes_possibles.append([lettre_cle])

    combinaisons_cles = product(*groupes_possibles)
    cles_potentielles = []

    for combinaison in combinaisons_cles:
        cle = "".join(combinaison)
        cles_potentielles.append(cle)

    return cles_potentielles,extrait